...
(defun pearl-paren-style--annotation-enabled-p ()
  "Return non-nil if annotation is currently enabled.
This is determined by whether there are annotation overlays present."
  (and pearl-paren-style-show-annotations
...
(defun pearl-paren-style--annotation-to-comment (closing-pos)
  "Convert annotation overlay at CLOSING-POS to comment text.
      ...
      (when result
        (let* ((annotation (car result))
               ...
               (comment-text (concat pearl-paren-style-annotation-comment-prefix
                                     (substring annotation 3))))
          (save-excursion
            (goto-char closing-pos)
            (forward-char 1) ; Move past the closing parenthesis
            (skip-chars-forward " \t")
            (let ((line-end (line-end-position)))
  ...
  (when (and pearl-paren-style-show-annotations
    ...
    (when (string-match "\\([0-9]+\\):\\([0-9]+\\) \\(.+\\)" comment-text)
      (let* ((open-line (string-to-number (match-string 1 comment-text)))
             ...
             (open-text (match-string 3 comment-text))
             (close-line (line-number-at-pos closing-pos))
             (line-distance (- close-line open-line))
             (annotation-text (format " ← %d:%d %s" open-line open-col open-text))
          ...
          (save-excursion
            ...
            (when (and (> (point) (point-min))
              ...
              (let ((closing-pos (1- (point)))
                    (comment-text (buffer-substring comment-end (line-end-position))))
...
END is the end position of the region."
  (setq pearl-paren-style--annotation-debounce-timer nil)
  (when (and (pearl-paren-style--annotation-enabled-p)
...
_LEN is the length of the change (unused)."
  (when (pearl-paren-style--annotation-enabled-p)
...
outside of Emacs sessions."
  ...
  (cond
   ...
   ((> (length pearl-paren-style--annotation-overlays) 0)
    (let ((converted 0))
      ;; First, clear any existing annotation comments to avoid duplicates
      (save-excursion
        (goto-char (point-min))
        (while (re-search-forward (concat ")\\s-*"
                                          (regexp-quote pearl-paren-style-annotation-comment-prefix))
                                  nil t)
          (let ((closing-pos (1- (match-beginning 0))))
            (when (not (pearl-paren-style--in-string-or-comment-p closing-pos))
              (save-excursion
                (goto-char closing-pos)
                (forward-char 1)
                  ...
                  (when (not (pearl-paren-style--in-string-or-comment-p closing-pos))
                    ...
                    (let ((spaces-start (point)))
                      ...
                      (when (looking-at (regexp-quote pearl-paren-style-annotation-comment-prefix))
                        (delete-region spaces-start (line-end-position)))))))))))
      (when (> converted 0)
        (pearl-paren-style--setup-change-hook))
